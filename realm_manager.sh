...
install_dependencies() {
    ...
    if command -v apt-get &> /dev/null; then
        ...
        apt-get update -qq
        apt-get install -y socat curl openssl cron
    elif command -v yum &> /dev/null; then
        # CentOS/RHEL 系统
        echo "检测到 CentOS/RHEL 系统，正在安装依赖..."
        yum install -y socat curl openssl cronie
        systemctl enable crond
        systemctl start crond
    elif command -v dnf &> /dev/null; then
        # Fedora 系统
        echo "检测到 Fedora 系统，正在安装依赖..."
        dnf install -y socat curl openssl cronie
        systemctl enable crond
        systemctl start crond
    else
...
install_acme_sh() {
    ...
    if curl -fsSL ${ACME_SH_INSTALL_URL} | sh -s email=admin@$(hostname -f); then
...
auto_request_certificate() {
    local domain="$1"
    local verify_method="$2"
    
    echo "正在为域名 ${domain} 自动申请 SSL 证书..."
    echo "------------------------------------------------------------"
    ...
    # 检查必要的依赖
    if ! command -v socat &> /dev/null; then
        echo -e "${G_YELLOW}检测到缺少 socat 工具，正在安装...${NC}"
        if ! install_dependencies; then
            echo -e "${G_RED}依赖安装失败，无法申请证书。${NC}"
            return 1
        fi
    fi
    ...
    case $verify_method in
        1|"http")
            ...
            if netstat -tlnp 2>/dev/null | grep -q ":80 "; then
                echo -e "${G_YELLOW}警告: 检测到80端口被占用，可能会影响证书申请。${NC}"
...
add_wss_rule() {
    ...
    # 检查证书状态
    local cert_status=$(check_certificate "$domain")
    case $cert_status in
        1)
            echo -e "${G_YELLOW}域名 ${domain} 没有有效的 SSL 证书。${NC}"
            echo "请选择证书获取方式:"
            echo " 1) 自动申请证书 (HTTP-01 验证)"
            echo " 2) 自动申请证书 (DNS-01 验证)"
            echo " 3) 跳过证书申请 (手动配置)"
            read -p "请选择 [1-3]: " cert_choice
            
            case $cert_choice in
                1)
                    echo -e "${G_GREEN}选择了 HTTP-01 自动验证方式${NC}"
                    if auto_request_certificate "$domain" "http"; then
                        echo -e "${G_GREEN}证书申请成功，继续创建 WSS 规则...${NC}"
                    else
                    ...
                    ;;
                2)
                    echo -e "${G_GREEN}选择了 DNS-01 自动验证方式${NC}"
                    if auto_request_certificate "$domain" "dns"; then
                        echo -e "${G_GREEN}证书申请成功，继续创建 WSS 规则...${NC}"
                    else
                        echo -e "${G_RED}证书申请失败，无法创建 WSS 规则。${NC}"
                        return
                    ...
                    ;;
                3)
                    echo -e "${G_YELLOW}跳过自动申请，请确保手动配置了有效的 SSL 证书。${NC}"
                    echo -e "${G_YELLOW}证书文件应位于: ${SSL_CERT_DIR}/${domain}/fullchain.pem${NC}"
                    echo -e "${G_YELLOW}私钥文件应位于: ${SSL_CERT_DIR}/${domain}/privkey.pem${NC}"
                    read -p "确认继续创建 WSS 规则？(y/n): " confirm_skip
                    if [[ "${confirm_skip}" != "y" && "${confirm_skip}" != "Y" ]]; then
                        echo "操作已取消。"
                        return
                    fi
                    # 创建证书目录以防不存在
                    ...
                    ;;
                *)
                    echo -e "${G_RED}无效选项，操作已取消。${NC}"
                    return
                    ;;
            esac
            ;;
        2)
            echo -e "${G_YELLOW}域名 ${domain} 的证书即将过期。${NC}"
            read -p "是否先续订证书？(y/n): " renew_cert
            if [[ "${renew_cert}" == "y" || "${renew_cert}" == "Y" ]]; then
                echo "正在续订证书..."
                if ${ACME_SH_PATH} --renew -d "$domain" --force; then
                    echo -e "${G_GREEN}证书续订成功！${NC}"
                else
                    echo -e "${G_YELLOW}证书续订失败，但将继续使用现有证书创建规则。${NC}"
                fi
            fi
            ;;
        0)
            echo -e "${G_GREEN}域名 ${domain} 已有有效的 SSL 证书，继续创建 WSS 规则...${NC}"
            ;;
    esac
    ...
    echo "SSL 证书: ${cert_dir}/fullchain.pem"
    echo "SSL 私钥: ${cert_dir}/privkey.pem"
    echo "=========================="
...
request_ssl_certificate() {
    ...
    # 调用自动申请证书函数
    if auto_request_certificate "$domain" "$verify_method"; then
        echo -e "${G_GREEN}SSL 证书申请完成！${NC}"
    else
        echo -e "${G_RED}SSL 证书申请失败！${NC}"
    fi
}
#!/bin/bash
...
check_system_dependencies() {
    ...
    # 检查必要的命令
    local required_commands=("curl" "openssl" "socat" "crontab")
    
    for cmd in "${required_commands[@]}"; do
        if command -v "$cmd" &> /dev/null; then
            echo -e "${G_GREEN}✓${NC} $cmd 已安装"
        else
            echo -e "${G_RED}✗${NC} $cmd 未安装"
            missing_deps+=("$cmd")
        fi
    done
    ...
    # 检查 acme.sh
    if check_acme_installation; then
        echo -e "${G_GREEN}✓${NC} acme.sh 已安装"
        local acme_version=$(${ACME_SH_PATH} --version 2>/dev/null | head -n1)
        echo "  版本: ${acme_version:-未知}"
    else
        echo -e "${G_YELLOW}!${NC} acme.sh 未安装"
    fi
    
    # 检查系统服务
    if systemctl is-enabled cron &>/dev/null || systemctl is-enabled crond &>/dev/null; then
        echo -e "${G_GREEN}✓${NC} Cron 服务已启用"
    else
        echo -e "${G_YELLOW}!${NC} Cron 服务未启用"
    fi
    
    echo "------------------------------------------------------------"
    
    if [[ ${#missing_deps[@]} -gt 0 ]]; then
        echo -e "${G_YELLOW}发现缺少以下依赖: ${missing_deps[*]}${NC}"
        read -p "是否现在安装缺少的依赖？(y/n): " install_confirm
        if [[ "${install_confirm}" == "y" || "${install_confirm}" == "Y" ]]; then
            install_dependencies
        fi
    else
        echo -e "${G_GREEN}所有必要依赖都已安装！${NC}"
    fi
}
