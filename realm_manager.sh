...
SSL_CONFIG_PATH="${REALM_CONFIG_DIR}/ssl.conf"
NGINX_PROXY_PATH="/etc/nginx/sites-available/realm-wss"
NGINX_PROXY_ENABLED="/etc/nginx/sites-enabled/realm-wss"
#!/bin/bash
...
add_tcp_rule() {
    ...
    if [[ "$remote_addr" == *":"* && "$remote_addr" != \[* ]]; then echo -e "${G_YELLOW}检测到IPv6地址，将自动添加括号。${NC}"; formatted_remote_addr="[${remote_addr}]"; else formatted_remote_addr="${remote_addr}"; fi
...
add_ws_rule() {
    ...
    # 输入验证
    if [[ -z "$listen_port" || -z "$remote_addr" || -z "$remote_port" ]]; then echo -e "${G_RED}错误: 端口和地址不能为空。${NC}"; return; fi
    if ! [[ "$listen_port" =~ ^[0-9]+$ && "$listen_port" -ge 1 && "$listen_port" -le 65535 ]]; then echo -e "${G_RED}错误: 本地监听端口无效。${NC}"; return; fi
    if ! [[ "$remote_port" =~ ^[0-9]+$ && "$remote_port" -ge 1 && "$remote_port" -le 65535 ]]; then echo -e "${G_RED}错误: 远程目标端口无效。${NC}"; return; fi
    
    if grep -q "listen = \"0.0.0.0:${listen_port}\"" ${REALM_CONFIG_PATH} 2>/dev/null; then echo -e "${G_RED}错误: 本地监听端口 ${listen_port} 已存在。${NC}"; return; fi
    
    local formatted_remote_addr
    if [[ "$remote_addr" == *":"* && "$remote_addr" != \[* ]]; then formatted_remote_addr="[${remote_addr}]"; else formatted_remote_addr="${remote_addr}"; fi
    local final_remote_str="${formatted_remote_addr}:${remote_port}"
...
EOF
    ...
    echo
    echo "您可以手动运行以下命令测试自动续订:"
    echo "$auto_renew_script"
}
#!/bin/bash
