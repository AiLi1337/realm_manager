...
auto_request_certificate() {
    ...
    case $verify_method in
        1|"http")
            ...
            # 使用更可靠的证书申请方式
            if ${ACME_SH_PATH} --issue -d "$domain" --standalone --httpport 80 --force --debug; then
                echo "证书申请成功，正在安装证书..."
        ...
        2|"dns")
            ...
            if ${ACME_SH_PATH} --issue -d "$domain" --dns --yes-I-know-dns-manual-mode-enough-go-ahead-please --debug; then
                echo
                ...
                if ${ACME_SH_PATH} --renew -d "$domain" --yes-I-know-dns-manual-mode-enough-go-ahead-please --debug; then
                    echo "证书申请成功，正在安装证书..."
                    if ${ACME_SH_PATH} --install-cert -d "$domain" \
                        --cert-file "${cert_dir}/cert.pem" \
                        --key-file "${cert_dir}/privkey.pem" \
...
show_menu() {
    ...
    echo "---- Realm WSS 中转一键管理脚本 (v2.2 修复版) ----"
    echo " 作者: AiLi1337 | WSS增强版 & 端口占用修复"
    echo
    echo "1. 安装 Realm"
    ...
    echo "-----------------------------------------"
    echo
    echo -e "${G_YELLOW}修复内容:${NC}"
    echo "• 修复 WSS 自动申请证书失效问题"
    echo "• 修复删除转发规则后端口占用问题"
    echo "• 增强证书申请的稳定性和错误处理"
    echo "• 优化端口清理和服务重启逻辑"
}
